...
set _=^&echo,  sdns&set _sip=219.76.4.4

if "%1"=="" (
  echo,Usage:%_% Domain [edns_client_subnet]%_% www.google.com%_% www.google.com 42.112.8.19&&goto :eof
) else (
  if "%2"=="" (for /f %%c in ('curl -sSm2 --retry 1 --no-keepalive ip.6655.com/ip.aspx') do set _ed=%%c) else set _ed=%2
)
...
for /f "delims=" %%c in ('curl -sSm2 -Gdname^=%1 --retry 1 --connect-to ::%_sip% --no-keepalive https://dns.google.com/resolve?edns_client_subnet^=%_ed%') do set #json=%%c
